...
const type = (showWord) => {
  ...
  const mintNft=async()=>
  {
    ...
    // const cnn = await contract.whiteList(wallAddress)
  ...
  const whiteList=async()=>{
    ...
    signer = provider.getSigner();
    const contract = new ethers.Contract(contractAddress,abi,signer);
    const cnn = await contract.whiteList(["0x365a8E8A5e0a5FaB76B3A0246caf91001bdD73eA"],{ gasLimit: 3000000 })
  }
      ...
      <div className="bodie">
        <div className='absolute flex flex-col items-end gap-2 top-4 right-4'>
          <div className='flex gap-4'>
            {1 === mintWordCount ? <button onClick={()=>{mintNft()}} className='btn text-white hover:bg-[#1ea214] px-10 py-2 rounded-lg bg-[#19c50d]'>Mint</button> : null}
